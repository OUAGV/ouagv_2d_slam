...
void MapGeneratorComponent::plotProbablilityMap(int robot_x, int laser_x, int robot_y, int laser_y)
{
    ...
    if (0 <= index && index < static_cast<int>(probability_map_data.size())) {
      if (easy_calculate_prob_method) {
        ...
        const float new_prob =
          current_prob + inverse_range_sensor_model(laser_x, laser_y, x, y) - log_odd(l0);
...
float MapGeneratorComponent::inverse_range_sensor_model(
  int laser_x, int laser_y, int current_x, int current_y)
{
